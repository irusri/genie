	...
	var data_p="id="+id;
        ...
        success: function (data) {
			if(JSON.parse(data)['error']!=null){
				$('#tab-container').hide();
				$('#title').html("<font color='red'>Incorrect "+header_genie_select_species_name+" id "+JSON.parse(data)['error']+'</font>');
				return false;
			}
			...
			$('#title').html(header_genie_select_species_name+"  gene model "+JSON.parse(data)['basic_data'][0].transcript_id);
			$('#start').html(JSON.parse(data)['basic_data'][0].transcript_start);
			$('#end').html(JSON.parse(data)['basic_data'][0].transcript_end);
			}else{
			$('#title').html(header_genie_select_species_name+" gene "+JSON.parse(data)['basic_data'][0].gene_id);
